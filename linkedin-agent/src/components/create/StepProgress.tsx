"use client";

interface StepProgressProps {
  steps: string[];
  current: number;
}

export default function StepProgress({ steps, current }: StepProgressProps) {
  return (
    <div className="rounded border border-[#1a1a1a] bg-[#0a0a0a] px-5 py-4 mb-6">
      <div className="flex items-center justify-between mb-4">
        <span className="text-[10px] font-semibold uppercase tracking-[0.14em] text-gray-600">
          Progress
        </span>
        <span className="font-mono text-[10px] text-gray-700">
          Step {current + 1} of {steps.length}
        </span>
      </div>

      <div className="flex items-start">
        {steps.map((label, i) => (
          <div key={label} className="flex items-start flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1.5 min-w-[72px]">
              <div
                className={`relative flex h-10 w-10 items-center justify-center rounded-full border-2 transition-all duration-500 ${
                  i < current
                    ? "border-gray-500 bg-gray-500/10"
                    : i === current
                      ? "border-gray-400 bg-gray-400/5"
                      : "border-[#1e1e1e] bg-[#111]"
                }`}
              >
                {i < current && (
                  <svg className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                  </svg>
                )}
                {i === current && (
                  <span className="text-[11px] font-bold text-gray-200">{i + 1}</span>
                )}
                {i > current && (
                  <span className="text-[11px] font-semibold text-gray-700">{i + 1}</span>
                )}
              </div>
              <span
                className={`text-[11px] font-medium hidden sm:block ${
                  i < current
                    ? "text-gray-400"
                    : i === current
                      ? "text-gray-300"
                      : "text-gray-700"
                }`}
              >
                {label}
              </span>
            </div>

            {i < steps.length - 1 && (
              <div className="flex-1 mx-1.5 h-px mt-5 rounded-full bg-[#1a1a1a] overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-700 ${
                    i < current
                      ? "w-full bg-gray-600"
                      : i === current
                        ? "w-1/2 bg-gradient-to-r from-gray-500 via-gray-400 to-gray-500 animate-shimmer"
                        : "w-0"
                  }`}
                  style={i === current ? { backgroundSize: "200% 100%" } : undefined}
                />
              </div>
            )}
          </div>
        ))}
      </div>
    </div>
  );
}
